...
class CheckIn {
  ...
  homeVisitStatus = false; // 首页访问
  signInPageAccessStatus = false; // 签到页访问
  ...
  async run() {
    ...
    try {
      ...
      await browser.open({args: ['--no-sandbox', '--disable-setuid-sandbox']});
      try {
        await browser.visitPage("/");
        this.homeVisitStatus = true;
      ...
      try {
        ...
        this.signInPageAccessStatus = true;
...
------事件埋点追踪-------
SDK状态: ${this.calledSdkSetting ? "加载成功" : "加载失败"}
成长API事件埋点: ${this.calledTrackGrowthEvent ? "调用成功" : "调用失败"}
OnLoad事件埋点: ${this.calledTrackOnloadEvent ? "调用成功" : "调用失败"}
-----------------------

------模拟访问------
掘金首页：页面访问${this.homeVisitStatus ? '成功' : '失败' } 
掘金每日签到：页面访问${this.signInPageAccessStatus ? '成功' : '失败' } 
-------------------
`.trim();
  }
}
